package services.dataCollectors.MergirafSemiCollector.mergeToolRunners
...
class MergirafSemiRunnerSC extends MergeToolRunner {
    private static final Path MERGIRAF_PATH = Paths.get('dependencies/mergiraf-semi-sc')

    MergirafSemiRunnerSC() {
        this.mergeToolName = 'mergiraf-semi-sc'
    }
    ...
    protected List<String> buildParameters(Path leftFile, Path baseFile, Path rightFile) {
        ...
        List<String> parameters = [
            binaryFileCommand, 'merge',  '--semistructured=diff3',
